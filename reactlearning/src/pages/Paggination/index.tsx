...
import { pagginationLessons } from "./lessons.ts";
...
export default function Paggination(_: HomeProps) {
    ...
    const TodoTile = (todo: any) => {
        return (
            <div className={style.tile} key={todo.id}>
                {todo.completed ? <div className={style.completed}>Completed</div> : <div className={style.notCompleted}>Not Completed</div>}
                <div>{todo.title}</div>
            </div>
        )
    }
    ...
    return (
        <div className={commonStyles.container}>
            ...
            <div className={commonStyles.vertical} aria-label="Lessons">
                {pagginationLessons.map((lesson, index) => {
